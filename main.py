...
def simplex_iteration(A_nb, A_b, C_nb, C_b, b, problem_type):
    ...
    # run new iteration
    return simplex_iteration(A_nb, A_b, C_nb, C_b, b, problem_type)
...
enter only number '1' or '2': """)
    ...
    # ans = str(optimal)
    # ans = ans[:ans.index('.') + accuracy + 1]
    # float(ans + ' ' + ans)
    optimal *= 10 ** accuracy
    optimal = int(optimal)
    optimal = float(optimal) / 10 ** accuracy
    # print(np.prototype.toFixed(optimal, accuracy))
    print(optimal)
